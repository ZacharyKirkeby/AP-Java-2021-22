  ...
  public void fixUnderwater() {
	  ...
	  for (int row = 0; row < pixelMap.length; row++){
		    for (int col = 0; col < pixelMap[0].length; col++) {
		    	...
		    	pixel.setRed(red*6);
				pixel.setGreen(245-green);
				pixel.setBlue(200-blue);
  }
	  }
  }
  ...
  public void mirrorHorizontalBotToTop() {
      ...
      for (int row = 0; row < pixels.length-1; row++){
        for (int col = 0; col < pixels[0].length-1; col++){
          ...
          if ((left.colorDistance(rightColor) > edgeDist) || (left.colorDistance(downColor) > edgeDist) ){
  	...
  	public void photoshop(Picture background) {
  		Pixel[][] pixels = getPixels2D();
  		Pixel[][] onlyZach = background.getPixels2D();
  		...
  		for(int row = 0; row < pixels.length-1; row++) {
  			for(int col = 0; col < pixels[0].length; col++) {
  				...
  				pixel2 = onlyZach[row][col];
  				color = pixel.getColor();
  				
  			if(row >= 375 && color.getRed() < 100 && color.getBlue() < 140 && color.getGreen() < 150) {
  			...
  			if((color.getGreen() >= 90) && !(color.getRed() >= 160) && !(color.getBlue() >= 200) ) {	
  					pixel.setColor(pixel2.getColor());
